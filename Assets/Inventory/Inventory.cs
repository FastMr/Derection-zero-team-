...
public class Inventory : MonoBehaviour {
	...
	public bool Add(Item item, GameObject itemObject) {
		...
		for (int j = 0; j < space; j++) {
			if (items [j] == null && itemObjects [j] == null) {
				...
				GameObject.Find ("Status").GetComponent<StatusScript> ().Message ("В инвентарь добавлен " + item.name);
	...
	public void AddOnBelt(Item beltItem, GameObject beltItemObject) {
		if (beltItems.Length > 4) {
			GameObject.Find ("Status").GetComponent<StatusScript> ().Message ("Недостаточно места!");
			return;
		}
		GameObject.Find ("Status").GetComponent<StatusScript> ().Message ("Одеваю на пояс " + beltItem);
		for (int i = 0; i < 4; i++) {
